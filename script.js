...
menu.addEventListener('click',()=>{const open=menu.getAttribute('aria-expanded')==='true';menu.setAttribute('aria-expanded',String(!open));menu.setAttribute('aria-label',open?'Open navigation':'Close navigation');nav.classList.toggle('open',!open);body.classList.toggle('locked',!open)});
nav.addEventListener('click',event=>{if(!event.target.matches('a'))return;menu.setAttribute('aria-expanded','false');nav.classList.remove('open');body.classList.remove('locked')});
const target=new Date('2026-12-19T10:30:00+02:00').getTime();function countdown(){const left=Math.max(0,target-Date.now()),values=[Math.floor(left/864e5),Math.floor(left/36e5)%24,Math.floor(left/6e4)%60,Math.floor(left/1e3)%60];['days','hours','minutes','seconds'].forEach((id,i)=>document.getElementById(id).textContent=String(values[i]).padStart(i?2:3,'0'))}countdown();setInterval(countdown,1000);
